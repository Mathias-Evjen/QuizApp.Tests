...
namespace QuizApp.Test.Controllers
{
    public class FlashCardControllerTests
    {
        ...
        [Fact]
        public async Task TestGetFlashCardsNotFound()
        {
            // arrange
            var mockFlashCards = new List<FlashCard>()
            {
            ...
            var mockFlashCardRepository = new Mock<IQuestionRepository<FlashCard>>();
            mockFlashCardRepository.Setup(repo => repo.GetAll(fc => fc.QuizId == mockQuizId)).ReturnsAsync((FlashCard[]?)null);
            var mockSerivce = new Mock<IFlashCardQuizService>();
            mockSerivce.Setup(service => service.PickRandomFlashCardColor()).Returns("blue");
            var mockLogger = new Mock<ILogger<FlashCardAPIController>>();
            var flashCardController = new FlashCardAPIController(
            ...
            Assert.Equal("FlashCards not found", notFoundResult.Value);
        }
        ...
        public async Task TestCreateDtoIsNull()
        {
            ...
            // act 
            var result = await flashCardController.Create(null);
        ...
        [Fact]
        public async Task TestCreateCreateFailed()
        {
            // arrange
            var mockFlashCardDto = new FlashCardDto
            {
        ...
        public async Task TestEditOk()
        {
            ...
            var mockReturnedFlashCard = new FlashCard
            {
                FlashCardId = 1,
                Question = "What is the capital of Norway?",
                Answer = "Bergen",
            ...
            returnedFlashCard.Should().BeEquivalentTo(mockReturnedFlashCard);
        }
